...
const BookingCard = ({ data }: { data: TProperty }) => {
  ...
  const { data: reservations, isLoading: reservationsLoading } = usePropertyReservations(data.id)
  ...
  const basePrice = useMemo(() => calculateBasePrice(data.price_per_night, nights), [data.price_per_night, nights])
  const serviceFee = useMemo(() => calculateServiceFee(basePrice), [basePrice])
  const totalPrice = useMemo(() => calculateTotalPrice(data.price_per_night, nights), [data.price_per_night, nights])
  const totalGuests = useMemo(
    () => watchedValues.guests.adults + watchedValues.guests.children,
    [watchedValues.guests.adults, watchedValues.guests.children],
  )

  const handlePopoverChange = useCallback((popover: PopoverType, isOpen: boolean) => {
  ...
  const getDateDisabled = useCallback(
    (field: "checkIn" | "checkOut") => (date: Date) => {
      ...
      if (field === "checkOut" && watchedValues.checkIn && date <= watchedValues.checkIn) return true
  ...
  const guestConfig = useMemo(
    () => [
      { key: "adults" as const, label: "Adults", desc: "Ages 13 or above", min: 1 },
      { key: "children" as const, label: "Children", desc: "Ages 2–12", min: 0 },
      { key: "infants" as const, label: "Infants", desc: "Under 2", min: 0 },
      { key: "pets" as const, label: "Pets", desc: null, min: 0, showServiceAnimalLink: true },
    ],
    [],
  )
            ...
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-6">
                  ₹{data.price_per_night} <span className="text-base font-normal">per night</span>
                </h2>
                          ...
                          <PopoverTrigger asChild>
                            <FormControl>
                              ...
                              >
                                ...
                                {activePopover === "guests" && <ChevronRight className="h-4 w-4 rotate-90" />}
                              </Button>
                            </FormControl>
                          </PopoverTrigger>
                          <PopoverContent className="w-[350px] p-4 rounded-xl" align="center">
                            <div className="space-y-6">
                              {guestConfig.map(({ key, label, desc, min, showServiceAnimalLink }) => (
                                <GuestSelector
                                  key={key}
                                  label={label}
                                  description={desc ?? undefined}
                                  value={watchedValues.guests[key]}
                                  onChange={(increment) => handleGuestChange(key, increment)}
                                  min={min}
                                  showServiceAnimalLink={showServiceAnimalLink}
                                />
                              ))}
                            </div>
                          </PopoverContent>
              ...
              {nights > 0 ? (
                ...
                <div className="text-center text-muted-foreground py-4">Select dates to see pricing</div>
              )}
              ...
              {!userId ? (
                ...
                <Button
                  ...
                  type="submit"
                  disabled={
                    bookPropertyMutation.isPending || !watchedValues.checkIn || !watchedValues.checkOut || nights === 0
                  }
                >
